...
function rowToMeta(r: Record<string, unknown>): ChartMetadata {
  return {
    ...
    kpi_config:    r.kpiConfig    as ChartMetadata['kpi_config']    ?? undefined,
    report_config: r.reportConfig as ChartMetadata['report_config'] ?? undefined,
    button_config: r.buttonConfig as ChartMetadata['button_config'] ?? undefined,
    date_filter:   r.dateFilter   as ChartMetadata['date_filter']   ?? undefined,
  }
}
...
export async function listGraficos(): Promise<ChartMetadata[]> {
  ...
  const rows = await sql`
    SELECT id, nome, descricao, categoria, chart_type, query_sql,
           ...
           cliente_ids, kpi_config, report_config, button_config, date_filter, created_at
    FROM graficos
...
export async function createGrafico(meta: ChartMetadata): Promise<ChartMetadata> {
  ...
  const rows = await sql`
    INSERT INTO graficos (
      ...
      refresh_seconds, timeout_seconds, axes, display, permissions, is_publico,
      cliente_ids, kpi_config, report_config, button_config, date_filter
    )
    VALUES (
